...
init_arg() {
  tell_object(this_player(), "Cloned: a little blue yak.\n");
  return 1;
}
...
init() {
  if(environment(this_object())->query_real_name() != "bart") {
    write("A little blue cow says: Moo. Make yer own little blue yak.\n");
    this_object()->destroy();
  }
  ::init();
